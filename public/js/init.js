...
function atribuiCookies(){
  if ($("#empresa_form").length >0){
    ...
    if(Cookies.get("formPj") !== undefined){
      ...
      if($("#whats_form").val() == "") $("#whats_form").val(formPj.telefone);
      if($("#email_form").val() == "") $("#email_form").val(formPj.email);
      if($("#empresa_form").val() == "") $("#empresa_form").val(formPj.empresa);
      if($("#cnpj_form").val() == "") $("#cnpj_form").val(formPj.cnpj);
    ...
    if(Cookies.get("formPf") !== undefined){
      ...
      //if($(".clinicaAtivo").length == 0) $("#"+formPf.clinica).addClass("clinicaAtivo");
    }
  }
}

function tags(nomeTag){
  gtag("event", nomeTag);
}
